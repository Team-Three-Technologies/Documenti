...
\setVerificators{Nenad Radulovic \\ & Filippo Compagno \\ & Francesco Balestro \\ & Bianca Zaghetto \\ & Sara Gioia Fichera \\ & Andrea Masiero}
\setApprovation{}
\setVersion{0.8}
\setType{Esterno}
...
\begin{document}
    ...
    \section*{Registro delle modifiche} {
        \begin{table}[h!]
            \rowcolors{2}{lightgray}{white}
                ...
                \hline
                \textbf{0.8} & 26/04/26 & Bianca Zaghetto & Programmatore & Nenad Radulovic & Aggiornamento requisiti e grafici \\
                \hline
                \textbf{0.7} & 26/04/26 & Francesco Balestro & Programmatore & Bianca Zaghetto & Stesura "Infrastructure layer" e "Classi per il rendering" \\
                \hline
    ...
    \section{Architettura} {
        ...
        \subsection{Comunicazione tra Main process e Renderer process}\label{sec:ipc-communication} {
            ...
            \\
            Il modello di comunicazione adottato è asincrono basato su Promise: il Renderer process invoca un metodo sul servizio \texttt{ElectronIPCService} (ad esempio \texttt{autolimport()}, \texttt{content()}, \texttt{searchDocuments()}) che, tramite IPC, richiede al Main process di eseguire l'operazione corrispondente. Il Main process, al completamento, restituisce una risposta standardizzata attraverso il DTO \texttt{IpcResponse<T>} (Sezione \ref{par:IpcResponse}), che incapsula sia i dati di successo che eventuali errori. Questo pattern garantisce un handling uniforme degli errori e la tipizzazione forte delle risposte.\\
            \\
            Per le operazioni di lunga durata (come l'importazione automatica o la generazione di anteprime), il Main process può inoltre inviare eventi di progresso o notifiche asincrone attraverso i canali IPC, a cui il Renderer process può sottoscriversi tramite il metodo \texttt{on()} del servizio.
        }
