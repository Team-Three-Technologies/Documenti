...
\setApprovation{Francesco Balestro, Andrea Masiero}
\setVersion{2.0}
\setType{Interno}
...
\section*{Registro delle modifiche}
    ...
    \begin{xltabular}{\textwidth}{|l|l|X|l|X|X|}
        ...
        \hline
        \endlastfoot
        \textbf{2.0} & 26/04/2026 & Francesco Balestro & Responsabile &  & Approvazione documento \\
        \hline
        \textbf{1.1} & 15/04/2026 & Bianca Zaghetto & Programmatore & Filippo Compagno & Convenzioni di naming \\
        \hline
        ...
        \subsubsection{Codifica}
            ...
            \paragraph{Norme di codifica}
                ...
                \begin{itemize}
                    ...
                    \item \textbf{Indentazione}: va rispettata la formattazione del codice per favorirne la leggibilità. Il gruppo utilizza lo strumento Prettier per standardizzare l'indentazione, 
                    in modo che le regole vengano applicate automaticamente.
                \end{itemize}

            \paragraph{Convenzioni di naming}

                Per garantire uniformità, leggibilità e manutenibilità del codice, il gruppo adotta le seguenti convenzioni di denominazione:

                \begin{itemize}
                    \item \textbf{Classi}: devono essere denominate utilizzando la notazione \textit{PascalCase}.\\
                    Esempio: \texttt{DipIndexParserImpl}
                
                    \item \textbf{Variabili e metodi}: devono essere denominati utilizzando la notazione \textit{camelCase}.\\
                    Esempio: \texttt{parseTagValue}
                
                    \item \textbf{Costanti}: devono essere denominate utilizzando la notazione \textit{SNAKE\_CASE} in maiuscolo.\\
                    Esempio: \texttt{DIP\_AUTO\_IMPORT}
                
                    \item \textbf{File}: devono seguire la seguente convenzione:
                    \begin{itemize}
                        \item struttura del nome del file nel formato:
                        \begin{center}
                            \texttt{nome.responsabilità.interface} \quad oppure \quad \texttt{nome.responsabilità.impl}
                        \end{center}
                        \item il nome viene scritto in \textit{kebab-case} se composto da più parole;
                    \end{itemize}
                    Esempi: \texttt{dip-index.parser.interface}, \texttt{metadata.parser.impl}
                \end{itemize}
